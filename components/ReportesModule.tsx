...
export const ReportesModule: React.FC = () => {
  ...
  const loadReports = async () => {
    setLoading(true);
    try {
      const data = await reportService.getDailyClosings();
      setReports(data || []);
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="max-w-[1400px] mx-auto space-y-8 animate-fade pb-24">
      ...
      <div className="bg-white p-10 border border-slate-200 rounded-[40px] shadow-sm flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Conectado a n8n & Supabase</p>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap justify-center gap-4">
          ...
          >
            <Terminal size={18}/> {showGuide ? 'Cerrar Terminal' : 'Consola n8n'}
          </button>
          <button onClick={loadReports} className="o-btn o-btn-primary gap-3 py-4 px-8 shadow-xl shadow-odoo-primary/20">
            <RefreshCw size={18} className={loading ? 'animate-spin' : ''}/> Sincronizar Ahora
          </button>
        </div>
      </div>
        ...
        <div className="bg-slate-900 rounded-[32px] p-8 animate-fade border border-slate-700 shadow-2xl overflow-hidden relative">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-400 text-xs font-medium mb-8 max-w-2xl">
              ¿Quieres verificar que n8n está enviando bien los datos? Copia este JSON y pégalo en tu nodo de prueba en n8n. Si funciona, aparecerá una tarjeta de "TEST" abajo inmediatamente.
            </p>
    ...
    "Visa": 200.00
              ...
              <div className="space-y-4">
                 <div className="bg-slate-800/50 p-5 rounded-2xl border border-slate-700">
                    <p className="text-[10px] font-black text-emerald-400 uppercase mb-2">Paso 1: Webhook / Odoo</p>
                    <p className="text-[11px] text-slate-300">n8n recibe la orden de Odoo o el cron de las 11:00 PM.</p>
                 </div>
                 <div className="bg-slate-800/50 p-5 rounded-2xl border border-slate-700">
                    <p className="text-[10px] font-black text-sky-400 uppercase mb-2">Paso 2: Transformación</p>
                    <p className="text-[11px] text-slate-300">Usa un nodo "Set" para mapear los campos igual que en el JSON de la izquierda.</p>
                 </div>
                 <div className="bg-slate-800/50 p-5 rounded-2xl border border-slate-700">
                    <p className="text-[10px] font-black text-purple-400 uppercase mb-2">Paso 3: Supabase (Upsert)</p>
                    <p className="text-[11px] text-slate-300">Usa la operación "Upsert" con Conflict Columns: "pos_id,fecha".</p>
                 </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          <div className="col-span-full py-24 text-center">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Consultando Supabase...</p>
          </div>
        ) : reports.length === 0 ? (
          ...
          <div key={rpt.id} className="bg-white border border-slate-200 rounded-[40px] overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="bg-slate-50 p-5 rounded-[24px] border border-slate-100">
                    <p className="text-[8px] font-black text-slate-400 uppercase mb-1.5 tracking-widest">Total Recaudado</p>
              ...
              {rpt.pagos && (
                <div className="pt-2">
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                     <Wallet size={14} className="text-odoo-primary"/> Mix de Medios de Pago
                   </p>
            ...
            <div className="px-8 py-4 border-t bg-slate-50/50 flex justify-between items-center text-[8px] font-bold text-slate-300 uppercase italic tracking-widest">
               ...
               <div className="flex items-center gap-1">
                 ...
                 <span className="text-emerald-600/60">DB Verified</span>
               </div>
            </div>
          </div>
        ))}
